...
const projects: ProjectBlogItem[] = [
  {
    ...
    dek: 'Joined 9 raw datasets across 96,478 orders to isolate late delivery impact on review scores and uncover an 82.54% GMV seller concentration risk.',
  ...
  },
  {
    ...
    dek: 'Traced root cause of a 30% Q1 revenue drop while order volume stayed flat through transaction-level and client-conversation audits, restoring run-rate by Q3.',
    tools: ['EXCEL', 'TRANSACTION AUDIT', 'SALES OPS', 'PROCESS CONTROLS'],
      ...
      <section className="project-blog-stream" aria-label="Featured Projects">
        {projects.map((proj) => (
          <Link
            ...
            id={`project-${proj.id}`}
          >
            {/* Topbar: Category Pill on left, Timeline Bar, Brand Logo on right */}
