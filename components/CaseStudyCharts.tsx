...
function initCharts(project: string) {
  if (typeof window === 'undefined') return;
  if (!window.Chart || !window.NarrativeCharts) return;

  const NC = window.NarrativeCharts;
  NC.applyDefaults();

  if (project === 'olist') {
    NC.gradientLineChart(
    ...
    NC.lorenzCurve('chart-olist-lorenz', lorenzX, lorenzY, {
      refX: 20,
      refY: 82.54,
      aspectRatio: 1.2,
    });
...
export default function CaseStudyCharts({ project }: CaseStudyChartsProps) {
  useEffect(() => {
    // If both libraries already loaded (e.g. navigating back to page), init immediately
    if (window.Chart && window.NarrativeCharts) {
      initCharts(project);
      return;
    }

    // Otherwise poll every 100ms until both are available (max 10s)
    let attempts = 0;
    const interval = setInterval(() => {
      ...
      if (window.Chart && window.NarrativeCharts) {
        ...
        clearInterval(interval);
        console.warn('[CaseStudyCharts] Timed out waiting for Chart.js / NarrativeCharts');
      }
    }, 100);
